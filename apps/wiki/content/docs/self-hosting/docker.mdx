...
```bash
docker-compose exec api yarn workspace @repo/db migrate:prod
```
...
```bash
docker-compose exec db pg_dump -U postgres plunk > backup.sql
```

### Restore Database

```bash
docker-compose exec -T db psql -U postgres plunk < backup.sql
```
